...
export const pollCommits = async (projectId: string) => {
    ...
    console.log(`Total fetched: ${commitHashes.length}`);
    ...
    const commitsToInsert = summaryResponse.map((res, index) => {
      ...
      console.warn(`⚠️ Failed to summarise commit ${unprocessedCommits[index]!.commitHash}:`, res.reason);
      return null;
    }).filter(Boolean); // filter out failed ones
  
    const commits = await db.commit.createMany({
      data: commitsToInsert as any,
      skipDuplicates: true, // 🔥 prevent Prisma crash on existing commitHash
    });
  
    console.log(`✅ Commits successfully inserted: ${commits.count}`);
    return commits;
  };
